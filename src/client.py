...
from src.tools.parsers import parse_junit_xml, parse_cobertura_xml, read_security_config, get_latest_code_changes
...
def main():
    ...
    print("[INFO] Fetching Git History...")
    git_changes = get_latest_code_changes(args.repo_root)
    # print(git_changes) # Debug
    ...
    ## MISSION
    Analyze the CI/CD pipeline data AND the recent code changes to produce a comprehensive decision report and release notes.
    ...
    **MEDIUM PRIORITY (Warning):**
    ...
    3. **release_notes**: A markdown string summarizing the changes for a CHANGELOG. 
       - Use "### Features", "###  Fixes", "### 🛠 Tech Debt" sections.
       - Base this ENTIRELY on the Git logs provided.
       
    4. **analysis_summary**: A rich markdown string with these sections:
    
       # Release Decision: [APPROVED/REJECTED]
       
       ## Executive Summary
       [2-3 sentence overview of the decision, key factors, and SUMMARY OF CHANGES]
    ...
    # Save Release Notes
    rn_content = result.get("release_notes", "No release notes generated.")
    with open(os.path.join(args.artifacts, "release_notes.md"), "w", encoding="utf-8") as f:
        f.write(rn_content)
